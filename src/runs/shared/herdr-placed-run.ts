...
export function discoverBridgeManifest(machine: HerdrMachineReference, runId: string, runtimeDir: string, options: { sshBin?: string; env?: NodeJS.ProcessEnv } = {}): { socketPath: string; nativeSessionId: string; packageVersion: string; protocol: number } {
	const script = `p="$1/manifest.json"; test -f "$p" && test ! -L "$p" && cat "$p"`;
	for (let attempt = 0; attempt < 100; attempt++) {
		const result = runHerdrRemoteCommand(machine, remoteShellCommand(script, [runtimeDir]), { ...options, timeout: 5_000, maxBuffer: 64 * 1024 });
...
export class HerdrPiSession implements ChildSession {
	...
	#snapshot: HerdrRunSnapshot; #identity: HerdrRunIdentity; #connection: HerdrForwardedConnection; bridgeForward: { close(): Promise<void> }; bridge: BridgeChannel; readonly requestedTools: string[] | undefined; readonly model: string | undefined; unsubscribeHerdr: () => void; #evidenceCursor = 0; #reconnecting?: Promise<void>; #reconnectFactory?: () => Promise<void>; #reconnectFailure?: Error;
	...
	constructor(identity: HerdrRunIdentity, connection: HerdrForwardedConnection, bridgeForward: { close(): Promise<void> }, bridge: BridgeChannel, requestedTools: string[] | undefined, model: string | undefined, unsubscribeHerdr: () => void, runtime: ChildRuntimeConfig, initialGit?: HerdrRemoteGitStatus, owner?: HerdrPlacedRunOwner) { this.owner = owner; this.#identity = identity; this.#connection = connection; this.bridgeForward = bridgeForward; this.bridge = bridge; this.requestedTools = requestedTools; this.model = model; this.unsubscribeHerdr = unsubscribeHerdr; this.runtime = runtime; this.supervisorDir = runtime.supervisorChannelDir; this.#snapshot = { connection: "connected", state: "ready", identity, revision: 0 }; this.#initialGit = initialGit; bridge.listeners.add((frame) => this.#frame(frame)); if (this.supervisorDir) { this.#replyTimer = setInterval(() => { const replies = path.join(this.supervisorDir!, "replies"); for (const name of fs.existsSync(replies) ? fs.readdirSync(replies) : []) { const file = path.join(replies, name); try { const value = JSON.parse(fs.readFileSync(file, "utf8")) as { message?: unknown; requestId?: unknown }; if (typeof value.message === "string" && typeof value.requestId === "string") void this.#operation("supervisor-reply", value.message, value.requestId).then(() => fs.rmSync(file, { force: true })); } catch {} } }, 50); this.#replyTimer.unref?.(); } }
	get identity(): HerdrRunIdentity { return this.owner?.identity ?? this.#identity; } set identity(value: HerdrRunIdentity) { this.#identity = value; if (this.owner) this.owner.identity = value; }
	get connection(): HerdrForwardedConnection { return this.owner?.connection ?? this.#connection; } set connection(value: HerdrForwardedConnection) { this.#connection = value; if (this.owner) this.owner.connection = value; }
	...
	async replaceTransport(input: { connection: HerdrForwardedConnection; bridgeForward: { close(): Promise<void> }; bridge: BridgeChannel; unsubscribe: () => void; paneId: string; cursor: number; state: HerdrRunSnapshot["state"] }) { if (this.#disposed) { input.unsubscribe(); input.bridge.close(); await input.bridgeForward.close(); await input.connection.close(); return; } const old = { connection: this.connection, bridgeForward: this.bridgeForward, bridge: this.bridge, unsubscribe: this.unsubscribeHerdr }; if (!this.owner) this.connection = input.connection; this.bridgeForward = input.bridgeForward; this.bridge = input.bridge; this.unsubscribeHerdr = input.unsubscribe; this.identity = { ...this.identity, paneId: input.paneId }; this.#reconnectFailure = undefined; for (const frame of input.bridge.frames) if (typeof frame.cursor === "number" && frame.cursor > this.#evidenceCursor) this.#frame(frame); input.bridge.listeners.add((frame) => this.#frame(frame)); for (const operation of this.#operations.values()) if (!operation.accepted && !operation.settled) { operation.settled = true; operation.reject(new Error("Remote operation was not accepted before transport loss; task state is unknown.")); } this.#evidenceCursor = Math.max(this.#evidenceCursor, input.cursor); this.#snapshot = { connection: "connected", state: input.state, identity: this.identity, revision: this.#snapshot.revision + 1 }; const ownerReplacement = this.owner?.replaceConnection({ connection: input.connection, unsubscribe: input.unsubscribe, paneId: input.paneId, state: input.state }); old.bridge.close(); await old.bridgeForward.close(); if (ownerReplacement) await ownerReplacement; else { old.unsubscribe(); await old.connection.close(); } this.armReconnect(this.#reconnectFactory!); }
	...
	async dispose() { if (this.#disposed) return; this.#disposed = true; if (this.#replyTimer) clearInterval(this.#replyTimer); this.bridge.close(); await this.bridgeForward.close(); if (this.owner) await this.owner.cleanup(this.#aborted); else { let cleanupError: unknown; try { if (this.#aborted) { const current = parseHerdrAgent(await this.connection.client.call<Record<string, unknown>>("agent.get", { target: this.identity.agentName }), "agent_info"); if (!ownsHerdrPane(current, this.identity.terminalId, this.identity.paneId)) throw new Error("Refusing cleanup because Herdr pane ownership changed."); await this.connection.client.call("pane.close", { pane_id: this.identity.paneId }); } } catch (error) { cleanupError = error; } finally { this.unsubscribeHerdr(); await this.connection.close(); } if (cleanupError) throw cleanupError; } this.#snapshot = { ...this.#snapshot, connection: "closed" }; }
	...
	get placementSnapshot() { return this.owner?.snapshot ?? this.#snapshot; }
}
...
export async function reconnectHerdrPiSession(session: HerdrPiSession, launch: ChildSessionLaunch, dependencies: { connect?: typeof connectHerdrMachine; discoverManifest?: typeof discoverBridgeManifest; createBridge?: (socketPath: string, runId: string) => BridgeChannel } = {}): Promise<void> {
	...
	const result = await boundedHerdrReconnect(session.identity, session.evidenceCursor, async () => {
		...
		try {
			...
			unsubscribe = await connection.client.subscribe(session.owner?.statusSubscriptions(check.paneId) ?? [{ type: "pane.agent_status_changed", pane_id: check.paneId }, { type: "pane.closed" }, { type: "pane.moved" }], (event) => { session.owner?.observeEvent(event); session.observeHerdr(event); }, (error) => { if (!generation.adopted) { generation.lost ??= error; return; } if (session.owner) session.owner.markConnectionUnknown(connection); void session.reconnect(); });
...
export async function createHerdrPiSession(launch: ChildSessionLaunch): Promise<ChildSession> {
	...
	try {
		...
		const session = new HerdrPiSession(identity, connection, bridgeForward, bridge, (configured.tools as string[] | undefined), typeof ready.model === "string" ? ready.model : undefined, () => {}, launch.runtime, ready.initialGit as HerdrRemoteGitStatus | undefined, owner); reconnectSession = session; observeHerdr = (event) => session.observeHerdr(event); session.armReconnect(() => reconnectHerdrPiSession(session, launch)); return session;
